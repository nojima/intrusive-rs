...
pub unsafe fn meld<Entry: Ord, const OFFSET: usize>(
    ...
    unsafe {
        ...
        if (*root1).left.is_null() || (*(*root1).left).s < (*(*root1).right).s {
            swap(&mut (*root1).left, &mut (*root1).right);
        }
        (*root1).s = if (*root1).right.is_null() { 0 } else { (*(*root1).right).s } + 1;
        (*root1).parent = parent;
    }
    root1
}
...
// Removes the given node from the heap and returns new root.
// O(log n)
pub unsafe fn unlink<Entry: Ord, const OFFSET: usize>(
    ...
    // node の親からのリンクを subtree へのリンクに置き換える
    ...
    // node が削除されたことで、node の祖先のノードについて leftist property が破れている可能性がある。
    // よって、node の祖先を調べて leftist property が再び満たされるように修正していく。
    let mut p = parent;
    while !p.is_null() {
        let sl = if (*p).left.is_null() { 0 } else { (*(*p).left).s };
        let sr = if (*p).right.is_null() { 0 } else { (*(*p).right).s };
        // leftist property が破れていたら修正する
        if sl < sr {
            swap(&mut (*p).left, &mut (*p).right);
        }
        let new_s = min(sl, sr) + 1;
        // 一度 s の値が変化しないノードがあれば、それ以降はずっと変化しないことが保証されるので、
        // ループを打ち切ってよい。この打ち切りをしないと計算量が O(n) になってしまう。
        if new_s == (*p).s {
            break;
        }
        (*p).s = new_s;
        ...
        for i in 0..10000 {
            ...
            match action {
                ...
                Action::Unlink(idx) => {
                    ...
                    root = unsafe {
                        unlink::<Entry, HOOK_OFFSET>(root, x_entry.as_mut().hook.get_mut())
                    };
                }
            }
        }
